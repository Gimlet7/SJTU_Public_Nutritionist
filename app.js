...
function renderOptionContent(question, key, fallback) {
  return question.optionBlocks?.[key] ? renderQuestionBlocks(question.optionBlocks[key], fallback) : text(fallback);
}
...
function renderPractice() {
  const session = state.session; const q = session.questions[session.index]; const selected = new Set(session.selected); const submitted = session.submitted; const correct = submitted && q.answers.length === selected.size && q.answers.every((answer) => selected.has(answer));
   const optionHtml = Object.entries(q.options).map(([key, content]) => { const isCorrect = submitted && q.answers.includes(key); const isWrong = submitted && selected.has(key) && !isCorrect; return `<div class="option ${selected.has(key) ? "selected" : ""} ${isCorrect ? "correct" : ""} ${isWrong ? "wrong" : ""}" data-option="${key}" role="button" tabindex="0" aria-pressed="${selected.has(key)}" aria-disabled="${submitted}"><span class="option-key">${key}</span><span class="option-content">${renderOptionContent(q, key, content)}</span></div>`; }).join("");
  ...
  const picker = `<details class="question-picker"><summary>题目选择 <strong>${session.index + 1} / ${session.questions.length}</strong></summary><div class="question-picker-panel"><div class="question-legend"><span><i class="unanswered"></i>未作答</span><span><i class="wrong"></i>错题</span><span><i class="passed"></i>已通过</span></div><div class="question-number-grid">${numberGrid}</div></div></details>`;
   layout(`<div class="practice-head"><button class="btn ghost" id="back-home">退出练习</button><div class="practice-nav"><button class="icon-btn" id="prev-question" title="上一题" aria-label="上一题" ${session.index === 0 ? "disabled" : ""}>&lt;</button><span class="progress">按顺序练习 · ${session.index + 1} / ${session.questions.length}</span><button class="icon-btn" id="next-question" title="下一题" aria-label="下一题" ${session.index === session.questions.length - 1 ? "disabled" : ""}>&gt;</button></div></div>${picker}<article class="panel question-panel"><div class="question-meta"><span class="badge">${q.type === "multiple" ? "多选题" : "单选题"}</span><span class="progress">${q.bank} 题库 · 原题 ${q.sourceQuestionNo}</span></div>${q.sharedStem ? `<div class="shared-stem">${q.sharedStemBlocks ? renderQuestionBlocks(q.sharedStemBlocks, q.sharedStem) : text(q.sharedStem)}</div>` : ""}<h2 class="question-stem">${q.stemBlocks ? renderQuestionBlocks(q.stemBlocks, q.stem) : text(q.stem)}</h2><div class="options">${optionHtml}</div>${feedback}<div class="practice-actions"><button class="btn ghost" id="bookmark">${state.bookmarks.has(q.id) ? "已收藏" : "收藏本题"}</button>${submitted ? `<button class="btn" id="next">${session.index === session.questions.length - 1 ? "查看结果" : "下一题"}</button>` : `<button class="btn" id="submit">提交答案</button>`}</div></article>`, "");
   const move = (delta) => { const nextIndex = session.index + delta; if (nextIndex < 0 || nextIndex >= session.questions.length) return; session.index = nextIndex; session.selected = []; session.submitted = false; saveProgress(); renderPractice(); };
   document.querySelector("#back-home").onclick = () => { saveProgress(); go("/home"); }; document.querySelector("#prev-question").onclick = () => move(-1); document.querySelector("#next-question").onclick = () => move(1); document.querySelector("#bookmark").onclick = async () => { if (state.bookmarks.has(q.id)) { state.bookmarks.delete(q.id); await remove("bookmarks", q.id); } else { state.bookmarks.add(q.id); await put("bookmarks", { id: q.id, createdAt: Date.now() }); } renderPractice(); };
  document.querySelectorAll("[data-jump]").forEach((button) => button.onclick = () => { session.index = Number(button.dataset.jump); session.selected = []; session.submitted = false; saveProgress(); renderPractice(); });
   document.querySelectorAll("[data-option]").forEach((button) => { const choose = () => { if (submitted) return; const key = button.dataset.option; if (q.type === "single") session.selected = [key]; else session.selected = selected.has(key) ? session.selected.filter((item) => item !== key) : [...session.selected, key]; renderPractice(); }; button.onclick = choose; button.onkeydown = (event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); choose(); } }; });
